...
function refreshData(){
    ...
    EGGS_TO_HATCH_1HIPSTER(function(eggs){
        eggstohatch1=eggs
    });
    getMyEggs(function(eggs){
        ...
        secondsuntilfull=eggstohatch1-eggs/lastNumHipster
        console.log('secondsuntilfull ',secondsuntilfull,eggstohatch1,eggs,lastNumHipster)
    ...
    getMyHipster(function(hipster){
        lastNumHipster=hipster
        var gfsdoc=document.getElementById('getfreehipster')
        if(hipster>0){
        ...
        var allnumhipster=document.getElementsByClassName('numhipster')
        for(var i=0;i<allnumhipster.length;i++){
            if(allnumhipster[i]){
                allnumhipster[i].textContent=translateQuantity(hipster,0)
            }
        }
        var productiondoc=document.getElementById('production')
        productiondoc.textContent=formatEggs(lastNumHipster*60*60)
    });
    updateBuyPrice()
    updateSellPrice()
	updateHipstermasterPrice()
	updateCurrentHipstermaster()
...
function updateEggNumber(eggs){
    var hatchhipsterquantitydoc=document.getElementById('hatchhipsterquantity')
    hatchhipsterquantitydoc.textContent=translateQuantity(eggs,0)
...
function liveUpdateEggs(){
    if(lastSecondsUntilFull>1 && lastNumEggs>=0 && lastNumHipster>0 && eggstohatch1>0){
        currentTime=new Date().getTime()
        if(currentTime/1000-lastHatchTime>eggstohatch1){
            return;
        }
        difference=(currentTime-lastUpdate)/1000
        additionalEggs=Math.floor(difference*lastNumHipster)
...
function updateHipstermasterPrice(){
    var hipstermasterpricedoc=document.getElementById('hipstermasterprice')
	getHipstermasterReq(function(req) {
		hipstermasterpricedoc.textContent = translateQuantity(req, 0);
	});
}


function updateCurrentHipstermaster(){
    var currenthipstermasterdoc=document.getElementById('currenthipstermaster')
    //eggstobuydoc.textContent='?'
	currenthipstermaster.textContent=ceoAddress()
}

function getFreeHipster2(){
    var ethtospenddoc=0.001//document.getElementById('freehipsterspend')
    weitospend=web3.toWei(ethtospenddoc,'ether')
    getFreeHipster(weitospend,function(){
...
function disableButtons(){
    var allnumhipster=document.getElementsByClassName('btn-lg')
    for(var i=0;i<allnumhipster.length;i++){
        if(allnumhipster[i]){
            allnumhipster[i].style.display="none"
        }
    }
}
function enableButtons(){
    ...
    for(var i=0;i<allnumhipster.length;i++){
        if(allnumhipster[i]){
            allnumhipster[i].style.display="inline-block"
        }
    }
}
